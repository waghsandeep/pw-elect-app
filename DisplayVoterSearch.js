...
const CSVTable = () => {
  ...
  useEffect(() => {
    const fetchCSVData = async () => {
      try {
        const response = await fetch(`${process.env.PUBLIC_URL}/final_merged_complete_voter_data.csv`); // Fetching from the public folder
  ...
  const filteredData = csvData.filter((row) => {
    ...
    const fullNameColumns = [
      ...
      row[headers.indexOf('EPIC_NO')],
        row[headers.indexOf('EPIC_NO')]// Include VCARDID column for searching
    ];
